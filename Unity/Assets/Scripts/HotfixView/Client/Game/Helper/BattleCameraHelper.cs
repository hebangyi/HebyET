...
namespace ET.Client
{
    public static class BattleCameraHelper
    {
        ...
        public static void CameraRotateUpdate(this UnitySceneCameraComponent self, long delTime)
        {
            var mainPlayer = MainPlayerHelper.GetCurrentWorldMainPlayer();
            if (mainPlayer == null)
            {
            ...
            // 0.5s 转 90度
            var angle = (int)(delTime * 1.0f / GameConstant.CameraRotationSpeed * 90);
            if (playerCacheDataComponent.CameraAngleOffSet < playerCacheDataComponent.TargetCameraAngleOffSet)
            {
                playerCacheDataComponent.CameraAngleOffSet = Math.Min(playerCacheDataComponent.CameraAngleOffSet + angle,
                    playerCacheDataComponent.TargetCameraAngleOffSet);
            }
            else
            {
                playerCacheDataComponent.CameraAngleOffSet = Math.Max(playerCacheDataComponent.CameraAngleOffSet - angle,
                    playerCacheDataComponent.TargetCameraAngleOffSet);
            }
