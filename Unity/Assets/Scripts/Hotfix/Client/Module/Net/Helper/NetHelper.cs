...
namespace ET.Client
{
    ...
    {
        // 注册router
        public static async ETTask<Session> CreateRouterSession(this NetComponent netComponent, IPEndPoint routerAddress, IPEndPoint address)
        {
            // TODO 本地的locationId生成
            uint localConn = (uint)(RandomGenerator.RandUInt32());
            uint recvLocalConn = await GetRouterAddress(netComponent, routerAddress, address, localConn, 0);
            ...
            routerSession.AddComponent<RouterCheckComponent, IPEndPoint>(routerAddress);
            return routerSession;
        }
        
        
        public static async ETTask<uint> GetRouterAddress(this NetComponent netComponent, IPEndPoint routerAddress, IPEndPoint address,
        uint localConn, uint remoteConn)
        {
            Log.Info($"start get router address: {netComponent.Root().Id} {address} {localConn} {remoteConn}");
            //return (RandomHelper.RandUInt32(), address);
            // outerAddressComponent routerAddressComponent = netComponent.Root().GetComponent<RouterAddressComponent>();
            // IPEndPoint routerInfo = routerAddressComponent.GetAddress();

            uint recvLocalConn = await netComponent.Connect(routerAddress, address, localConn, remoteConn);

            Log.Info($"finish get router address: {netComponent.Root().Id} {address} {localConn} {remoteConn} {recvLocalConn} {routerAddress}");
