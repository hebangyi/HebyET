...
namespace ET.Client
{
    ...
    public static partial class RouterAddressComponentSystem
    {
        ...
        {
            string url = $"http://{self.RouterManagerHost}:{self.RouterManagerPort}{GameConstant.EntryServerServerListUri}?v={RandomGenerator.RandUInt32()}";
        ...
        {
            int v = account.Mode(self.Info.Accounts.Count);
            string address = self.Info.Accounts[v];
            string[] ss = address.Split(':');
            IPAddress ipAddress = IPAddress.Parse(ss[0]);
