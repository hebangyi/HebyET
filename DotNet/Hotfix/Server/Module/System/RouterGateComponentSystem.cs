            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                switch (node.Status)
                {
                    ...
                    case RouterStatus.Msg:
                        // 比session超时应该多10秒钟
                        if (timeNow > node.LastRecvOuterTime + GameConstant.HttpSessionTimeoutTime + 10 * 1000)
                        {
        ...
        private static void RecvOuterHandler(this RouterGateComponent self, int messageLength, long timeNow, IKcpTransport transport)
        {
            ...
            switch (flag)
            {
                case KcpProtocalType.RouterReconnectSYN:
                {
                    ...
                    {
                        Log.Warning(
                            $"kcp router router reconnect connectId diff, maybe router count too less: {connectId} {routerNode.ConnectId} {routerNode.SyncIpEndPoint} {(IPEndPoint)self.IPEndPoint}");
                ...
                case KcpProtocalType.RouterSYN:
                {
                    ...
                    {
                        Log.Warning(
                            $"kcp router router connect connectId diff, maybe router count too less: {connectId} {routerNode.ConnectId} {routerNode.SyncIpEndPoint} {(IPEndPoint)self.IPEndPoint}");
                        break;
                    }
